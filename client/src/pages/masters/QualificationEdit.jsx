...
const QualificationEdit = () => {
  ...
  const qualifications = {
    1: { id: 1, name: 'HSC', description: 'High School' },
    2: { id: 2, name: 'SSC', description: 'Secondary School' },
  };

  useEffect(() => {
    const selectedQualification = qualifications[id];
    if (selectedQualification) {
      setQualification(selectedQualification);
      setFormState(selectedQualification);
    } else {
  ...
  const handleSave = () => {
    console.log('Saving qualification:', formState);
    navigate('/master/qualification-master');
  };

  const handleDelete = () => {
    console.log('Deleting qualification:', formState.id);
    navigate('/master/qualification-master');
  };
